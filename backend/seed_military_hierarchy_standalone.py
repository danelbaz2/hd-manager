...
# MongoDB connection
MONGO_URI = os.getenv('MONGO_URI', 'mongodb://localhost:27017/hd_manager')
...
def seed_military_hierarchy():
    ...
    try:
        ...
        existing = db.military_hierarchy.find_one({})
        if existing:
            print("⚠️  Military hierarchy already exists. Skipping seed.")
            print("   To re-seed, delete the collection first or run with --force")
            return
        
        # Insert the hierarchy
        result = db.military_hierarchy.insert_one({
