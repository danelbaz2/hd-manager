...
const COLOR_OPTIONS = [
    ...
    { label: 'Green', value: 'bg-emerald-100 text-emerald-600 border-emerald-200' },
    { label: 'Orange', value: 'bg-orange-100 text-orange-600 border-orange-200' },
    { label: 'Red', value: 'bg-red-100 text-red-600 border-red-200' },
    { label: 'Slate', value: 'bg-slate-100 text-slate-600 border-slate-200' },
    { label: 'Pink', value: 'bg-pink-100 text-pink-600 border-pink-200' },
    { label: 'Indigo', value: 'bg-indigo-100 text-indigo-600 border-indigo-200' },
    { label: 'Cyan', value: 'bg-cyan-100 text-cyan-600 border-cyan-200' },
    { label: 'Teal', value: 'bg-teal-100 text-teal-600 border-teal-200' },
];
...
const INITIAL_CONTACTS = [
    { id: 1, name: 'תמיכה טכנית', role: 'חיצוני', phone: '050-0000000', email: 'support@example.com' },
    { id: 2, name: 'ספק שרתים', role: 'תשתיות', phone: '052-1111111', email: 'cloud@example.com' },
];
...
const WorkersSettings = ({ users, setUsers, isDarkMode }) => {
    const [editingId, setEditingId] = useState(null);
    const [formData, setFormData] = useState({ name: '', role: '', iconColor: COLOR_OPTIONS[0].value });

    const handleEdit = (user) => {
        setEditingId(user.id);
        setFormData({ name: user.name, role: user.role, iconColor: user.iconColor });
    };
    ...
    const handleSave = () => {
        ...
        setFormData({ name: '', role: '', iconColor: COLOR_OPTIONS[0].value });
    };

    return (
        <div className="space-y-6">
            <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                <h3 className="font-bold mb-4 flex items-center gap-2"><User size={18} /> {editingId ? 'עריכת עובד' : 'הוספת עובד חדש'}</h3>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div className="space-y-1">
                        <label className="text-xs font-bold opacity-70">שם מלא</label>
                        <input
                            type="text"
                            className={`w-full p-2 rounded-lg border outline-none ${isDarkMode ? 'bg-slate-900 border-slate-600' : 'bg-white border-slate-300'}`}
                            value={formData.name}
                            onChange={e => setFormData({ ...formData, name: e.target.value })}
                        />
                    </div>
                    <div className="space-y-1">
                        <label className="text-xs font-bold opacity-70">תפקיד</label>
                        <input
                            type="text"
                            className={`w-full p-2 rounded-lg border outline-none ${isDarkMode ? 'bg-slate-900 border-slate-600' : 'bg-white border-slate-300'}`}
                            value={formData.role}
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold opacity-70">צבע</label>
                        <div className="flex gap-1 flex-wrap">
                            {COLOR_OPTIONS.map((opt, i) => (
                                <button
                                    key={i}
                                    type="button"
                                    onClick={() => setFormData({ ...formData, iconColor: opt.value })}
                                    className={`w-6 h-6 rounded-full border-2 ${opt.value.split(' ')[0]} ${formData.iconColor === opt.value ? 'border-black dark:border-white scale-110' : 'border-transparent opacity-70'}`}
                                />
                            ))}
                        </div>
                    </div>
                    <button
                        ...
                        className="px-4 py-2 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 h-10"
                    >
            ...
            <div className="space-y-2">
                {users.map(user => (
                    <div key={user.id} className={`flex items-center justify-between p-3 rounded-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                        <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${user.iconColor}`}>
                                <User size={14} />
                            </div>
                            <div>
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={() => handleEdit(user)} className="p-2 hover:bg-blue-50 text-blue-600 rounded-lg dark:hover:bg-blue-900/30 transition-colors"><Edit2 size={16} /></button>
                            <button onClick={() => handleDelete(user.id)} className="p-2 hover:bg-red-50 text-red-600 rounded-lg dark:hover:bg-red-900/30 transition-colors"><Trash2 size={16} /></button>
                        </div>
                    </div>
                ))}
            </div>
...
const ContactsSettings = ({ contacts, setContacts, isDarkMode }) => {
    const [editingId, setEditingId] = useState(null);
    const [formData, setFormData] = useState({ name: '', role: '', phone: '', email: '' });

    const handleEdit = (contact) => {
        setEditingId(contact.id);
        setFormData({ name: contact.name, role: contact.role, phone: contact.phone, email: contact.email });
    };
    ...
    return (
        <div className="space-y-6">
            <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                <h3 className="font-bold mb-4 flex items-center gap-2"><Users size={18} /> {editingId ? 'עריכת איש קשר' : 'הוספת איש קשר'}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input placeholder="שם מלא" className={`p-2 rounded-lg border outline-none ${isDarkMode ? 'bg-slate-900 border-slate-600' : 'bg-white border-slate-300'}`} value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                    <input placeholder="תפקיד/תיאור" className={`p-2 rounded-lg border outline-none ${isDarkMode ? 'bg-slate-900 border-slate-600' : 'bg-white border-slate-300'}`} value={formData.role} onChange={e => setFormData({ ...formData, role: e.target.value })} />
                    <input placeholder="טלפון" className={`p-2 rounded-lg border outline-none ${isDarkMode ? 'bg-slate-900 border-slate-600' : 'bg-white border-slate-300'}`} value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} />
                    <input placeholder="אימייל" className={`p-2 rounded-lg border outline-none ${isDarkMode ? 'bg-slate-900 border-slate-600' : 'bg-white border-slate-300'}`} value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} />
                </div>
                <div className="mt-4 flex justify-end">
                    <button onClick={handleSave} disabled={!formData.name} className="px-6 py-2 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
                        {editingId ? 'שמור שינויים' : 'הוסף איש קשר'}
                    </button>
                </div>
            </div>
            ...
            <div className="space-y-2">
                {contacts.map(contact => (
                    <div key={contact.id} className={`flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border gap-4 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                        <div className="flex items-center gap-4">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300`}>
                                <User size={18} />
                            </div>
                            <div>
                                <div className="font-bold">{contact.name}</div>
                                <div className="text-xs opacity-70 mb-1">{contact.role}</div>
                        ...
                        <div className="flex items-center gap-2 self-end sm:self-center">
                            <button onClick={() => handleEdit(contact)} className="p-2 hover:bg-blue-50 text-blue-600 rounded-lg dark:hover:bg-blue-900/30 transition-colors"><Edit2 size={16} /></button>
                            <button onClick={() => handleDelete(contact.id)} className="p-2 hover:bg-red-50 text-red-600 rounded-lg dark:hover:bg-red-900/30 transition-colors"><Trash2 size={16} /></button>
                        </div>
                    </div>
                ))}
            </div>
...
const TagsSettings = ({ tags, setTags, isDarkMode }) => {
    const [editingId, setEditingId] = useState(null);
    const [formData, setFormData] = useState({ label: '', color: COLOR_OPTIONS[0].value });
    ...
    const handleSave = () => {
        if (editingId) {
            ...
            setTags([...tags, { id: `tag-${Date.now()}`, ...formData }]);
        }
        setFormData({ label: '', color: COLOR_OPTIONS[0].value });
    };

    return (
        <div className="space-y-6">
            <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                <h3 className="font-bold mb-4 flex items-center gap-2"><Tag size={18} /> {editingId ? 'עריכת תגית' : 'הוספת תגית חדשה'}</h3>
                <div className="flex flex-col sm:flex-row gap-4 items-end">
                    <div className="space-y-1 flex-1 w-full">
                        <label className="text-xs font-bold opacity-70">שם התגית</label>
                        <input
                            type="text"
                            className={`w-full p-2 rounded-lg border outline-none ${isDarkMode ? 'bg-slate-900 border-slate-600' : 'bg-white border-slate-300'}`}
                        ...
                        <div className="flex gap-1 flex-wrap">
                            {COLOR_OPTIONS.map((opt, i) => (
                                <button
                                    ...
                                    className={`w-6 h-6 rounded-full border-2 ${opt.value.split(' ')[0]} ${formData.color === opt.value ? 'border-black dark:border-white scale-110' : 'border-transparent opacity-70'}`}
                                />
                            ))}
                        </div>
                    </div>
                    <button
                        ...
                        className="px-4 py-2 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed h-10 w-full sm:w-auto"
                    >
                        {editingId ? 'שמור' : 'הוסף'}
                    </button>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {tags.map(tag => (
                    <div key={tag.id} className={`flex items-center justify-between p-3 rounded-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                        <span className={`text-xs font-bold px-2 py-1 rounded border ${tag.color}`}>
                            {tag.label}
                        </span>
                        <div className="flex items-center gap-2">
                            <button onClick={() => handleEdit(tag)} className="p-1.5 hover:bg-blue-50 text-blue-600 rounded-lg dark:hover:bg-blue-900/30 transition-colors"><Edit2 size={14} /></button>
                            <button onClick={() => handleDelete(tag.id)} className="p-1.5 hover:bg-red-50 text-red-600 rounded-lg dark:hover:bg-red-900/30 transition-colors"><Trash2 size={14} /></button>
                        </div>
                    </div>
                ))}
            </div>
...
const SettingsModal = ({ isOpen, onClose, isDarkMode, users, setUsers, tags, setTags, contacts, setContacts }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className={`relative w-full max-w-4xl h-[80vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row ${isDarkMode ? 'bg-slate-800 text-white' : 'bg-white text-slate-900'
                }`}>

                {/* Sidebar Navigation */}
                <div className={`w-full md:w-64 p-4 border-b md:border-b-0 md:border-l flex-shrink-0 ${isDarkMode ? 'bg-slate-900/50 border-slate-700' : 'bg-slate-50 border-slate-100'}`}>
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-xl font-bold flex items-center gap-2"><Settings className="text-blue-500" /> הגדרות מערכת</h2>
                    ...
                    <nav className="flex flex-row md:flex-col gap-2 overflow-x-auto md:overflow-visible">
                        <button
                            ...
                            className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all whitespace-nowrap ${activeTab === 'workers'
                        ...
                        <button
                            ...
                            className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all whitespace-nowrap ${activeTab === 'contacts'
                                ? 'bg-blue-500 text-white shadow-md font-bold'
                                : isDarkMode ? 'text-slate-400 hover:bg-slate-800' : 'text-slate-500 hover:bg-slate-100'
                                }`}
                        >
                            <Users size={18} /> אנשי קשר
                        </button>
                        <button
                            onClick={() => setActiveTab('tags')}
                            className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all whitespace-nowrap ${activeTab === 'tags'
                                ? 'bg-blue-500 text-white shadow-md font-bold'
                                : isDarkMode ? 'text-slate-400 hover:bg-slate-800' : 'text-slate-500 hover:bg-slate-100'
                                }`}
                        >
                ...
                <div className="flex-1 flex flex-col h-full overflow-hidden relative">
                    <button onClick={onClose} className="absolute top-4 left-4 p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/10 hidden md:block">
                        <X size={20} />
                    </button>

                    <div className="flex-1 overflow-y-auto custom-scrollbar p-6 pt-12 md:pt-6">
                        {activeTab === 'workers' && (
                            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                                <h2 className="text-2xl font-bold mb-6">ניהול עובדים</h2>
                                <WorkersSettings users={users} setUsers={setUsers} isDarkMode={isDarkMode} />
                            </div>
                        )}
                        {activeTab === 'contacts' && (
                            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                                <h2 className="text-2xl font-bold mb-6">אנשי קשר מערכתיים</h2>
                                <ContactsSettings contacts={contacts} setContacts={setContacts} isDarkMode={isDarkMode} />
                            </div>
                        )}
                        {activeTab === 'tags' && (
                            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                                <h2 className="text-2xl font-bold mb-6">ניהול תגיות משימה</h2>
                                <TagsSettings tags={tags} setTags={setTags} isDarkMode={isDarkMode} />
                            </div>
                        )}
                    </div>
                </div>
...
// ... WeeklyCalendar ...
const WeeklyCalendar = ({ currentDate, tasks, isDarkMode, onTaskClick, users }) => {
    ...
    return (
        <div className={`h-full flex flex-col rounded-2xl border overflow-hidden shadow-sm ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
            ...
            <div className="flex-1 relative overflow-y-auto custom-scrollbar p-2">
                ...
                <div className="grid grid-cols-7 gap-y-2 relative z-10" style={{ gridAutoRows: 'minmax(45px, auto)' }}>
                    {processedTasks.map(task => {
                        const responsible = task.responsibleUsersId && task.responsibleUsersId.length > 0 ? users.find(u => u.id === task.responsibleUsersId[0]) : null;
                        return (
                            <div
                                key={task.id}
...
const MissionModal = ({ mission, isOpen, onClose, isDarkMode, users, tags }) => {
...
const MissionRow = ({ mission, isDarkMode, onClick, users, tags }) => {
    const statusLabel = getStatusLabel(mission.status);
    const tag = tags.find(t => t.id === mission.tagId);
    const responsible = users.find(u => u.id === mission.responsibleUsersId[0]);
...
const KanbanTask = ({ task, isDarkMode, onDragStart, onClick, users }) => {
    const responsible = users.find(u => u.id === task.responsibleUsersId[0]);
    return (
...
const KanbanColumn = ({ title, status, tasks, isDarkMode, onDrop, onDragOver, icon: Icon, colorClass, onTaskClick, users }) => {
    return (
        <div onDrop={(e) => onDrop(e, status)} onDragOver={onDragOver} className={`flex-1 min-w-[300px] md:min-w-0 rounded-2xl p-4 flex flex-col h-full border ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-1">
                {tasks.map(task => (<KanbanTask key={task.id} task={task} isDarkMode={isDarkMode} onDragStart={(e, id) => e.dataTransfer.setData('taskId', id)} onClick={() => onTaskClick(task)} users={users} />))}
            </div>
        </div>
    );
};
...
export default function App() {
    ...
    return (
        <div className={`h-screen max-h-screen font-sans flex transition-colors duration-300 overflow-hidden ${isDarkMode ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-900'}`} dir="rtl">
            <MissionModal mission={selectedMission} isOpen={!!selectedMission} onClose={handleCloseModal} isDarkMode={isDarkMode} users={users} tags={tags} />
            ...
            <main className="flex-1 flex flex-col h-full relative">
                <header className={`relative h-20 flex-shrink-0 flex items-center justify-between px-8 shadow-sm z-30 ${isDarkMode ? 'bg-slate-800' : 'bg-white'}`}>
                    <div className="flex items-center gap-4">
                        {currentPage === 'home' && timeRange === 'day' && (
                        ...
                        {isUserMenuOpen && (
                            <div className={`absolute top-full left-0 mt-2 w-56 rounded-xl shadow-xl border overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 z-50 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
                                <div className="p-2 space-y-1">
                                    ...
                                    <button onClick={() => setIsSettingsModalOpen(true)} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-slate-700 text-slate-200' : 'hover:bg-slate-50 text-slate-700'}`}><Settings size={18} /><span className="text-sm font-medium">הגדרות</span></button>
                                </div>
                    ...
                    {currentPage === 'home' && (
                        <div className="flex flex-col h-full">
                            ...
                            <div className="flex-1 overflow-hidden relative">
                                {timeRange === 'week' && (
                                    viewMode === 'list' ? (
                                        <WeeklyCalendar currentDate={currentDate} tasks={filteredMissions} isDarkMode={isDarkMode} onTaskClick={handleMissionClick} getUserById={getUserById} />
                                    ) : (
                                ...
                                {timeRange === 'day' && viewMode === 'list' && (
                                    <div className="h-full overflow-auto pb-4 custom-scrollbar">
                                        ...
                                        <div className="space-y-1">
                                            {filteredMissions.length > 0 ? (filteredMissions.map((m) => (<MissionRow key={m.id} mission={m} isDarkMode={isDarkMode} onClick={() => handleMissionClick(m)} users={users} tags={tags} />))) : (
                    ...
                    {currentPage === 'missions' && selectedUser && (
                        <div className="h-full overflow-x-auto overflow-y-hidden">
                            <div className="flex h-full gap-6 w-full">
                                <KanbanColumn title="פתוח" status="open" tasks={missions.filter(t => t.status === 'open' && t.responsibleUsersId.includes(selectedUser.id))} isDarkMode={isDarkMode} onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} icon={AlertCircle} colorClass="text-emerald-500" onTaskClick={handleMissionClick} users={users} />
                                <KanbanColumn title="בטיפול" status="in_progress" tasks={missions.filter(t => t.status === 'in_progress' && t.responsibleUsersId.includes(selectedUser.id))} isDarkMode={isDarkMode} onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} icon={Clock} colorClass="text-amber-500" onTaskClick={handleMissionClick} users={users} />
                                <KanbanColumn title="סגור" status="closed" tasks={missions.filter(t => t.status === 'closed' && t.responsibleUsersId.includes(selectedUser.id))} isDarkMode={isDarkMode} onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} icon={CheckCircle2} colorClass="text-slate-500" onTaskClick={handleMissionClick} users={users} />
                            </div>
                        </div>
                    )}
                </div>
