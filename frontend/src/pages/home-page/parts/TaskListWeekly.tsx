...
// Get user by ID
const getUserById = (userId: string, users: UserData[]): UserData | undefined => {
...
const TaskListWeekly: React.FC<TaskListWeeklyProps> = ({
    ...
    // Calculate grid position for each task
    const getGridPosition = (task: Task): { colStart: number; colSpan: number } => {
        const taskStart = new Date(task.date || 0);
        const taskEnd = task.deadline ? new Date(task.deadline) : new Date(taskStart);
    ...
    // Process tasks and assign visual rows (to avoid overlap)
    const processedTasks: ProcessedTask[] = weekTasks
        .map((t) => ({ ...t, ...getGridPosition(t), visualRow: 0 }))
        .sort((a, b) => a.colStart - b.colStart);

    // Layout algorithm to assign rows without overlap
              ...
              `}
                ...
                `}
                            ...
                            >
                                {/* Status Bar */}
                                <div
                                    className={`absolute top-0 bottom-0 right-0 w-1.5 ${getStatusColor(
                                        task.status
                                    )}`}
                                />
