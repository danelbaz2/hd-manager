...
const getStatusLabel = (status?: string) => {
  switch (status) {
    ...
    case "pending_approval":
      return { label: "ממתין לאישור", color: "bg-purple-100 text-purple-700" };
    case "completed":
...
const TaskBrief: React.FC<TaskBriefProps> = ({ tasks }) => {
  ...
  return (
    <div
      className={`rounded-2xl border p-3 flex flex-col h-full ${
        isDarkMode
                      ...
                      <div className="flex gap-1 shrink-0">
                        {task.primaryTagIds
                          ...
                          .map((tagId: string, i: number) => {
                            const tag = primaryTags.find((t) => t.id === tagId);
                            return (
                              <span
                                key={i}
