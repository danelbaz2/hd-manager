...
export const MentionList: React.FC<MentionListProps> = ({
  ...
  // Determine positioning style based on whether position prop is provided
  ...
  const getPositionClass = () => {
    if (position) {
      return "rounded-xl border shadow-2xl backdrop-blur-sm overflow-hidden z-[9999]";
    }
    return "absolute bottom-full mb-2 right-0 w-72 rounded-xl border shadow-2xl backdrop-blur-sm overflow-hidden z-50";
  };
  ...
  // Scrollbar styling class matching UpdateTeam
  const scrollbarClass = isDarkMode ? "dark-scrollbar" : "light-scrollbar";
  ...
  const renderContent = () => {
    ...
    // Reset refs array for current contacts
    itemRefs.current = [];
    ...
    return (
      ...
      >
        ...
        {/* Contact List with scrollbar styling */}
        <div
          ref={listRef}
          className={`max-h-48 overflow-y-auto ${scrollbarClass}`}
        >
  ...
  // Use portal for fixed position rendering (when position is provided)
  if (position) {
    return createPortal(content, document.body);
  }

  // Regular rendering for relative positioning
  return content;
};
