...
// Uses useRef to avoid re-render loops with object content
const useCachedContent = <T,>(content: T, isOpen: boolean): T => {
  const cachedRef = React.useRef<T>(content);
  const prevIsOpenRef = React.useRef(isOpen);

  // Only update cache when modal opens (isOpen transitions from false to true)
  // or when already open and content changes
  if (isOpen && (!prevIsOpenRef.current || prevIsOpenRef.current)) {
    cachedRef.current = content;
  }

  prevIsOpenRef.current = isOpen;

  return isOpen ? content : cachedRef.current;
};

const ConfirmModal: React.FC<ConfirmModalProps> = (props) => {
  const { isOpen, onConfirm, onCancel, isDarkMode } = props;
    ...
    <ModalOverlay isOpen={isOpen} onClose={onCancel} maxWidthClass="max-w-md">
      ...
      >
        ...
        <div className="flex items-center justify-center gap-3 mb-4">
          <div
            className={`p-3 rounded-full ${isDarkMode ? config.iconBg.dark : config.iconBg.light}`}
          >
            <HeaderIcon
              size={24}
              className={
                isDarkMode ? config.iconColor.dark : config.iconColor.light
              }
            />
          </div>
        </div>
