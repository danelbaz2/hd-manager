...
import TagSelector from "../../manage-contact/components/TagSelector";
        ...
        <div className="flex flex-wrap items-center gap-4" dir="rtl">
          ...
          <div className="min-w-[200px]">
            <TagSelector
              ...
              selectedTagIds={formData.primaryTagId ? [formData.primaryTagId] : []}
              ...
              emptyText="אין קטגוריות זמינות"
              onSelectSingle={(primaryId) => onFormChange({ ...formData, primaryTagId: primaryId })}
              onToggleTag={() => {}}
              onRemoveTag={() => onFormChange({ ...formData, primaryTagId: "" })}
              usePortal
              hideSelector
            />
          </div>
          ...
          <input
            ...
            disabled={isSaving}
            className={`flex-1 min-w-[180px] px-4 py-2.5 rounded-lg border text-right text-sm transition-colors ${
              isDarkMode
                ? "bg-slate-700 border-slate-600 text-white placeholder-slate-400"
                : "bg-white border-slate-300 text-slate-800 placeholder-slate-400"
            } focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed`}
          />
          ...
          <button
            onClick={onSubmit}
            disabled={isSaving}
            className="flex items-center gap-2 px-6 py-2.5 rounded-lg bg-blue-500 hover:bg-blue-600 text-white font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isSaving ? (
